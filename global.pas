...
str.Add('</Entities>');
   ...
   if (assigned(tilemap)) then
    ...
    {
    str.Add('<Setup>');
    str.Add('<MapImage>'+tilemap.FImage.name+'</MapImage>');
...
begin
 ...
 isselect:=false;
    ...
    FSprite.RenderEx(x,y,RadToDeg(angle),HScale,VScale);
    FSprite.GetBoundingBoxEx(x,y,degtorad(angle),HScale,VScale,FBound);
    if isselect then
